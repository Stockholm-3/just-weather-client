...
json_t *weather_client_get_weather_by_city(WeatherClient *client,
  ...
  if (region && strlen(region) > 0) {
    ...
    if (region_encoded) {
      snprintf(url + len, sizeof(url) - len, "&region=%s", region_encoded);
      free(region_encoded);
    }
  }
...
json_t *weather_client_search_cities(WeatherClient *client, const char *query,
  ...
  normalize_string_for_cache(query, normalized_query, sizeof(normalized_query));
...
static json_t *make_request(WeatherClient *client, const char *url,
  ...
  if (!result) {
    if (error) {
      char err_msg[256];
      snprintf(err_msg, sizeof(err_msg), "JSON parse error: %s", json_err.text);
